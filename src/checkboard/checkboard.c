...
/*   Updated: 2023/12/20 18:50:24 by apriego-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	get_color_sphere(t_vec3 p_hit, t_world *world)
{
	...
	// return (color);
	t_uv uv = get_spherical_map(&p_hit, &world->type.sp->center, world->type.sp->radius);
	uv.u *= 4;
	uv.v *= 2;
...
{
	t_uv uv = get_planar_map(&p_hit, &world->type.pl->normal, &world->type.pl->center);
	return (checker_color(uv, world->materia.color));
}
...
t_color	get_color_cone(t_vec3 p_hit, t_world *world)
{
	...
	{
		t_uv uv = get_cylinder_map(&p_hit, &world->type.cn->dir, &world->type.cn->center, world->type.cn->height);
		uv.u *= 6;
		uv.v *= 2;
		return (checker_color(uv, world->materia.color));
	}
	...
	{
		t_uv uv = get_planar_map(&p_hit, &world->type.cn->dir, &world->type.pl->center);
		uv.u *= 6;
		uv.v *= 2;
		return (checker_color(uv, world->materia.color));
	}
...
t_color	get_color_cylinder(t_vec3 p_hit, t_world *world)
{
	...
	{
		t_uv uv = get_cylinder_map(&p_hit, &world->type.cy->dir, &world->type.cy->center, world->type.cy->height);
		uv.u *= 6;
		uv.v *= 2;
		return (checker_color(uv, world->materia.color));
	}
	...
	{
		t_uv uv = get_planar_map(&p_hit, &world->type.cy->dir, &world->type.cy->center);
		uv.u *= 10;
		uv.v *= 2;
		return (checker_color(uv, world->materia.color));
	}
