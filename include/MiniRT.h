...
/*   Updated: 2024/01/31 18:36:51 by apriego-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int						bitmap_bumpmap_case(t_scene *scene, t_world *sp,
							char **split, int pos);
int						bitmap_or_bumpmap_case(t_scene *sc, t_world *sp,
							char **s, int pos);
...
bool					hit_cylinder(const t_ray *ray, t_objects obj,
							t_hit *rec);
bool					hit_disk_cone(const t_ray *ray, t_objects obj,
							t_hit *rec);
...
t_color					get_cn_chess(t_vec3 *p_hit, t_world *objs,
							t_point3 *cent);
t_color					get_cn_bit(t_vec3 *p_hit, t_world *objs, t_point3 *cent,
							t_img_tex *texture);
t_color					get_cy_chess(t_vec3 *p_hit, t_world *objs,
							t_point3 *cent);
...
int						check_texture_sphere(t_scene *scene, t_world *sp,
							char **split);
int						check_texture_plane(t_scene *scene, t_world *pl,
							char **split);
int						check_texture_cylinder(t_scene *scene, t_world *cy,
							char **split);
int						check_texture_cone(t_scene *scene, t_world *cn,
							char **split);
...
t_color					render_edit_mode(t_scene *scene, const t_ray *r,
							t_hit *hit);
...
bool					calc_hard_shadows(t_world *objs, t_ray *r_light,
							t_hit *hit_rec);
t_color					calc_reflected_color(t_scene *scene, t_hit *hit_rec,
							const t_ray *r, int ray_depth);
void					draw_outlines(t_scene *scene);
void					apply_normal_map(t_hit *hit_rec);
t_color					sum_lights_color(t_scene *scene, t_light *lights,
							t_hit *hit_rec, const t_ray *r);
...
t_slider				init_slider(int min_value, int max_value, int *value,
							uint16_t length);
void					draw_slider(void *mlx_ptr, void *mlx_win,
							t_slider *slider);
